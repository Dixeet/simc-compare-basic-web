...
function displayResult(simcInput, combinations){
    var simcLines = simcInput.split('\n');
...
function cleanCommentsSimc(simcLines){
    var tmpSimc = [];
    simcLines.forEach(function (line) {
        if(line[0] !== "#"){
            tmpSimc.push(line);
        }
    });
...
function getCombinationString(combinations, name){
    var str = '';
    combinations.forEach(function (comb) {
        var strCopy = '\n\ncopy=';
        var strCombName = '';
        var strPart = '';
        comb.forEach(function (part) {
            var partialPartName = getPartialPartName(part.name);
            if(!!strCombName) {
                strCombName += "_&_"
            }
            strCombName += partialPartName;
            strPart += "\n" + part.line;
        });
        strCopy += strCombName + ',' + name;
        str += strCopy + strPart;
    });
    return str;
}

function getPartialPartName(partName){
    var name = partName.replace(/'/g, "");
    var spltPartName = partName.split(" ");
    if (spltPartName.length > 1) {
        name = spltPartName[0] + "-" + spltPartName[spltPartName.length - 1];
    }
    return name;
}

function getCharName(simcLines){
    var classes = [
        'deathknight',
        'demonhunter',
        'druid',
    ...
    simcLines.some(function (line) {
        var classFound = false;
        classes.forEach(function (classe) {
            if (line.indexOf(classe) !== -1){
                classFound = true;
            }
        });
        if (classFound){
